...
~80 curated providers (`CATALOG_VERSION`, now **13** — Financial Datasets (`X-API-KEY`) joined on 2026-09-15; Exa, Crustdata and Aviato are included, and
...
grant (see [local-run](../architecture/local-run.md)). `CATALOG_VERSION` is now **13**. An `unsupported:true` block is first-class: it tells the analyzer
